...
class MCP23017Board:
    ...
    def write_pin(self, pin, value):
        pin = str(pin).upper()
        if len(pin) != 3 or pin[0] != "P" or pin[1] not in ["A", "B"] or pin[2] not in "01234567":
...
def MCPActor(address, bus):
    global mcp
    logger.info("Start MCP23017 Actor on I2C bus %s address %s", bus, hex(address))
    try:
...
class MCP23017(CBPiExtension):
    ...
    async def MCP23017_Settings(self):
        ...
        settings = [
            ("MCP23017_Address", "0x20", "MCP23017 I2C bus address, e.g. 0x20. Change requires reboot"),
            ("MCP23017_Bus", "1", "MCP23017 I2C bus number, usually 1 on Raspberry Pi. Change requires reboot"),
        ]

        # Optional global aliases. These names are shown in logs and can be copied into the Actor field "PinName".
        # The GPIO selection remains PA0..PB7, because CBPi4 builds the dropdown when the plugin is loaded.
        for pin in PINS:
            settings.append((f"MCP23017_{pin}_Name", pin, f"Optional display name/label for MCP23017 pin {pin}. Example: Pump 1, Heater, Valve Mash."))
        ...
        for key, default, description in settings:
            ...
            if plugin_update is None or plugin_update != self.version or self.cbpi.config.get(key, None) is None:
                try:
                    await self.cbpi.config.add(key, current, type=ConfigType.STRING, description=description, source=self.name)
                except Exception as e:
                    logger.warning("Unable to update config %s: %s", key, e)

        if plugin_update is None or plugin_update != self.version:
            try:
                await self.cbpi.config.add(update_key, self.version, type=ConfigType.STRING, description="MCP23017 Plugin Version", source="hidden")
            except Exception as e:
...
@parameters(
    [
        Property.Select(label="GPIO", options=PINS, description="Hardware pin on MCP23017: PA0-PA7 or PB0-PB7"),
        Property.Text(label="PinName", configurable=True, description="Optional custom display name, e.g. Pump 1, Heater, Valve Mash"),
...
class MCP23017Actor(CBPiActor):
    ...
    def _pin_label(self):
        custom = str(self.props.get("PinName", "") or "").strip()
        if custom:
            return f"{custom} ({self.gpio})"
        try:
            global_name = self.cbpi.config.get(f"MCP23017_{self.gpio}_Name", self.gpio)
            if global_name and global_name != self.gpio:
                return f"{global_name} ({self.gpio})"
        except Exception:
            pass
        return self.gpio
    ...
    async def on_start(self):
        ...
        self.state = False
        self.label = self._pin_label()

        if mcp is not None:
            mcp.write_pin(self.gpio, self.off_value)
            logger.info("MCP23017 Actor %s initialized on %s", self.id, self.label)
        else:
            logger.error("MCP23017 is not initialized. Actor %s cannot set GPIO %s", self.id, self.label)

    async def on(self, power=None):
        self.power = power if power is not None else 100
        await self.set_power(self.power)
        logger.info("ACTOR %s ON - %s", self.id, self.label)
        if mcp is not None:
            mcp.write_pin(self.gpio, self.on_value)
        self.state = True

    async def off(self):
        logger.info("ACTOR %s OFF - %s", self.id, self.label)
        if mcp is not None:
            mcp.write_pin(self.gpio, self.off_value)
        self.state = False
